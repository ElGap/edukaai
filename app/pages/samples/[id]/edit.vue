    ...
    <div class="mb-6 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold mb-2">
          Edit Sample #{{ route.params.id }}
        </h1>
        <p class="text-secondary">Update your training sample details.</p>
      </div>
      <NuxtLink :to="`/samples/${route.params.id}`" class="btn-secondary">
        ← Cancel
      </NuxtLink>
    </div>
...
const loadNavigation = async (sampleId) => {
  console.log('Loading navigation for sample:', sampleId, 'dataset:', sample.value?.dataset_id || sample.value?.datasetId);
