...
export default defineEventHandler(async (event) => {
  try {
      ...
      case 'json':
        exportData = {
          ...
          stats: {
            ...
            avgQuality: filteredSamples.reduce((sum, ex) => sum + (ex.qualityRating || 3), 0) / filteredSamples.length
          }
        }
    ...
    const safeDatasetName = datasetName.toLowerCase().replace(/[^a-z0-9]/g, '_').replace(/_+/g, '_').substring(0, 30)
